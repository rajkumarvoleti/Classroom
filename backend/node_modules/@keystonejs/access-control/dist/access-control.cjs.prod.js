...
const checkSchemaNames = ({
  ...
  const matchingNames = utils.intersection(schemaNames, accessTypes);
  ...
  if (typeof access === 'object') {
    ...
    const providedNameCount = utils.intersection(accessKeys, schemaNames).length;
  ...
  const keyedBySchemaName = typeof access === 'object' && utils.intersection(Object.keys(access), schemaNames).length === Object.keys(access).length;
  return keyedBySchemaName;
};

function parseCustomAccess({
  defaultAccess,
  ...
  const fullAccess = keyedBySchemaName ? _objectSpread2(_objectSpread2({}, utils.defaultObj(schemaNames, defaultAccess)), access) // Access keyed by schemaName
  : utils.defaultObj(schemaNames, access); // Access not keyed by schemaName

  const fullParsedAccess = _objectSpread2(_objectSpread2({}, fullAccess), {}, {
    internal: true
  });

  Object.values(fullParsedAccess).forEach(access => {
    if (typeof access !== 'boolean' && typeof access !== 'function' && typeof access !== 'object') {
      throw new Error(`Expected a Boolean, Object, or Function for custom access, but got ${typeof access}`);
    }
  });
...
function parseListAccess({
  ...
  const parseAndValidate = (access = {}) => {
    if (typeof access === 'boolean' || typeof access === 'function') {
      return utils.defaultObj(accessTypes, access);
    } else if (typeof access === 'object') {
      const _access = access;

      if (Object.keys(utils.pick(_access, accessTypes)).length === 0) {
      ...
      return _objectSpread2(_objectSpread2({}, utils.defaultObj(accessTypes, defaultAccess)), utils.pick(_access, accessTypes));
    } else {
  ...
  const fullParsedAccess = _objectSpread2(_objectSpread2({}, schemaNames.reduce((acc, schemaName) => _objectSpread2(_objectSpread2({}, acc), {}, {
    ...
    internal: utils.defaultObj(accessTypes, true)
  });
...
async function validateAuthAccessControl({
  ...
  const operation = 'auth'; // Either a boolean or an object describing a where clause
...
exports.parseCustomAccess = parseCustomAccess;
...
exports.validateCustomAccessControl = validateCustomAccessControl;
